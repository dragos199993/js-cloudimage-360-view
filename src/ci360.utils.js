...
const get360ViewProps = (image) => ({
  list: attr(image, "list") || attr(image, "data-list"),
  ...
  filenameX:
    ...
    "image-{index}.jpg",
  filenameY:
    attr(image, "filename-y") ||
    attr(image, "data-filename-y") ||
    "image-y-{index}.jpg",
  imageList:
  ...
  amountY: parseInt(
    attr(image, "amount-y") || attr(image, "data-amount-y") || 0,
    10
  ),
  imageOffset: parseInt(
    attr(image, "image-offset") || attr(image, "data-image-offset")
  ),
  ...
  onMouseLeave:
    attr(image, "on-mouse-leave") || attr(image, "data-on-mouse-leave"),
  toStartPointerZoom:
    attr(image, "to-start-pointer-zoom") ||
  ...
  maxScale: parseFloat(
    attr(image, "max-scale") || attr(image, "data-max-scale") || 100,
    10
  ),
  ...
  ciTransformation:
    attr(image, "transformation") || attr(image, "data-transformation"),
...
const normalizeZoomFactor = (event, pointerZoomFactor) => {
  ...
  const zoomFactor =
    scrollEvent < 125 ? -pointerZoomFactor * 10 : -pointerZoomFactor;
